...
以中性的「青い子」為吉祥物，對應青色申告 75 萬控除（令和 9 年分以降，需 e-Tax + 優良電子帳簿；同時支援既存 65 萬）的個人開發工具。
自用優先，視情況以 OSS 形式公開。
...
## 8. 青色申告控除の硬性要求（65 万 / 75 万 共用）

實作必須滿足下列條件：

- [x] 複式簿記
- [x] 貸借対照表 + 損益計算書
- [x] **e-Tax 電子申告 OR 優良な電子帳簿保存**（兩者擇一、否則只有 55 萬→令和 9 年分以降 10 萬）
...
| 路線 | 控除額 | 適用年度 |
|---|---|---|
| e-Tax + 優良な電子帳簿 | **75 万** | 令和 9 年分（2027）以降 |
| e-Tax + 複式簿記（優良電子帳簿なし） | 65 万 | 据置 |
| 紙申告 + 複式簿記 | 令和 8 年分まで 55 万 / 令和 9 年分以降 10 万に縮小 | — |

### 路線選擇

**aoiko の現行スタンス**：「e-Tax + 複式簿記」を主路線とし 65 萬控除を確実に取得、
さらに「優良な電子帳簿」要件にも段階的に対応して 75 萬控除路線を解禁する。

「優良な電子帳簿」要件（aoiko 関連分）：
- 訂正・削除履歷 → ✓ reverseEntry 既に対応
- 帳簿間相互関連性 → ✓ 仕訳⇔補助元帳の参照あり
- 検索機能（取引年月日 / 取引金額 / 取引先）→ 日付・摘要・取引先は OK、金額検索は要追加（Issue #18 で対応）
- 範圍與組合検索 → JournalList の複合フィルタで対応
