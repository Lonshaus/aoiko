...
- Tables that continue off to the right now say so: the scrollbar is always shown as a slim bar, and the edges carry a fade where the content continues
- In the vendor list, Delete now always sits in the same place at the right-hand end

### Fixed

- Cancelling the confirmation for switching the filing method did not put the selection back. The screen kept showing the option you had picked while the old one was still in force, and saving from there wrote the old one
