...
describe('mapKoa220Values（青色申告決算書・不動産所得用 第1頁）', () => {
  ...
  test('礼金・権利金等（不動産）は礼金欄、雑収入（不動産）は追加科目欄に分かれる', () => {
    ...
    // 賃貸料 ANF00080・礼金 ANF00090・追加科目名 ANF00050・追加科目金額 ANF00100
    expect(out.ANF00080).toBe('1200000');
    expect(out.ANF00090).toBe('200000');
    expect(out.ANF00050).toBe('雑収入');
    expect(out.ANF00100).toBe('30000');
  });
    ...
    const out = mapKoa220Values(
      ctx({
        ...
        personalDeductions: withRealEstate({
          businessScale: true,
          landLoanInterestAmount: D(30000),
        }),
      }),
    );
    ...
    const repeats = mapKoa220RepeatedValues(
      ctx({
        fixedAssets: [
          realEstateAsset({
            incomeType: 'business',
            realEstateDetail: { propertyType: '貸家', address: 'X', annualRent: '1' },
          }),
        ],
      }),
    );
