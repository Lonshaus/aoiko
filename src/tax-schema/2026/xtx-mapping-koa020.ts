...
//
// ⚠ 申告者情報（氏名・住所・税務署）はここではなく IT部から IDREF で第一表へ反映される。
// 本モジュールが扱うのは第一表「事業」部分の直接値 leaf。所得控除・税額（ABB00420〜ABB01040）
// は ctx.personalDeductions がある場合のみ出力し、無ければ利用者が e-Tax 上で補完する。
// 白色申告では青色申告特別控除を出さず、事業所得は white-return-income.ts の補正後の値から
// さらに事業専従者控除を差し引いた値を使う。
