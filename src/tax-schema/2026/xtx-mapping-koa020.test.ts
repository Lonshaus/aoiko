...
describe('mapKoa020LeafValues / mapKoa020RepeatedValues（白色・事業専従者控除、issue #307）', () => {
  ...
  test('事業所得は専従者控除後の値、配偶者控除は専従者のため0になる', () => {
    ...
    // 合計所得金額314万円は336万円以下 → 基礎控除88万円
    expect(out.ABB00550).toBe('880000');
    // 所得控除合計＝基礎控除88万円のみ（社保等は全て0、配偶者控除は0）
    expect(out.ABB00560).toBe('880000');
    // 課税される所得金額＝314万−88万＝226万（千円未満の端数無し）
    expect(out.ABB00580).toBe('2260000');
    // 税額＝226万×10%−97,500円＝128,500円（195万円超330万円以下の令和8年分速算表）
    expect(out.ABB00590).toBe('128500');
  });
