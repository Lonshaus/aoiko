...
describe('buildXtxDocument (2 段式 ID/IDREF 文書モデル / 参照ファイル準拠)', () => {
  ...
  test('toYymmdd は西暦を令和の era/yy/mm/dd 複合型に変換する', () => {
    expect(toYymmdd('2026-06-30')).toBe('<gen:era>5</gen:era><gen:yy>8</gen:yy><gen:mm>6</gen:mm><gen:dd>30</gen:dd>');
    expect(toYymmdd('2027-01-01')).toBe('<gen:era>5</gen:era><gen:yy>9</gen:yy><gen:mm>1</gen:mm><gen:dd>1</gen:dd>');
  });
