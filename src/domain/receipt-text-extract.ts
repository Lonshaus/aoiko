...
// 右端を止める。止めないと、OCR が 1 桁多く読んだときに先頭 13 桁を切り出して
// そのまま通してしまう（実測。同じ領収書を離れて撮ると `T7180301016951` が
// `T71803010169511` で返った）。形式は合っているので利用者は誤りに気付けない。
// 桁数が違うなら採用せず、空欄にして人手に委ねる。
