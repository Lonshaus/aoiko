...
describe('commitImport', () => {
  test('creates one entry with two lines per valid row', async () => {
    ...
    const result = await commitImport({ ...KNOWN_INFO, fileHash: 'hash-1' }, rows);
  ...
  test('税区分未指定なら従来どおり税率 0', async () => {
    ...
    const counter = (await db.journalLines.toArray()).find((l) => l.accountCode === '5130');
  ...
  test('skipped rows are excluded', async () => {
    ...
    const result = await commitImport({ ...KNOWN_INFO, fileHash: 'hash-skip' }, rows);
  ...
  test('rows missing counterpart are excluded', async () => {
    ...
    await expect(commitImport({ ...KNOWN_INFO, fileHash: 'hash-empty' }, rows)).rejects.toThrow(
      /登録対象/,
    );
  });
  ...
  test('rejects duplicate fileHash', async () => {
    ...
    await expect(
      commitImport({ ...KNOWN_INFO, fileHash: 'shared-hash', fileName: 'second.csv' }, rows),
  ...
  test('all entries link back to ImportBatch via sourceImportId', async () => {
    ...
    const result = await commitImport({ ...KNOWN_INFO, fileHash: 'hash-link' }, rows);
