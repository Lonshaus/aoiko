...
type BackupStatus =
  ...
  // wrapper 版と、iOS の bookmark が失効した場合の両方で使う（回復動線が同じため）。
...
class BackupManager {
  ...
  private async initAdapter(): Promise<void> {
    ...
    // WKWebView に showDirectoryPicker は無いため、wrapper 版はネイティブ層を先に見る。
    // これが無いと macOS / iPadOS / iOS の app は opfs 止まりになり、同期フォルダへの
    // 自動書き出しに到達できない。
  ...
  private async initNative(adapter: NativeFolderBackupAdapter): Promise<void> {
    ...
    // token が解決できない場合（iOS の bookmark 失効・フォルダの削除や移動）も、ネイティブ層に
