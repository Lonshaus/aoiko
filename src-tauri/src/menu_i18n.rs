...
//
// 繁體中文の文言は macOS 自身が出しているものに合わせてある（Ventura 13.3 の Safari の
...
/// メニューに出る文字列の種類。
// Windows と macOS でメニューの構成そのものが違うため、どちらの環境でも使われない
...
pub enum Key {
    ...
    Minimize,
    Zoom,
    // ここから Windows 用。同じ操作でも既定の綴りが macOS と違い（元に戻す／取り消す、
    // 檢視／顯示方式）、`&` の後ろ 1 文字がニーモニックとして下線付きで出る。
    // macOS 側の文言に `&` を混ぜられないので、種類ごと分ける。
    WinFileMenu,
    WinEditMenu,
    WinViewMenu,
    WinHelpMenu,
