...
// 見分ける手段（userAgent 等）は WKWebView だと macOS と iPadOS の区別が付かない。
...
// src-init/index.js が印刷と外部リンクの実装を出し分けるためのフラグ。iOS/iPadOS だけ
// plugin-aoiko-native の print_page / open_in_app へ回す。
#[tauri::command]
fn is_ios() -> bool {
    cfg!(target_os = "ios")
}
// WKWebView 上の window.print() は例外を投げる（tauri#3066）。印刷スタイルは公開 repo に
    ...
    #[cfg(target_os = "macos")]
    {
        window
            .with_webview(|webview| {
                ...
                // with_webview のクロージャは主スレッドで実行されるため AppKit を直接触れる。
                unsafe {
                    let wk = &*wk;
    ...
    #[cfg(target_os = "windows")]
    {
        window
            .with_webview(|webview| {
                ...
                // controller() が返すのは基底の ICoreWebView2 で、ShowPrintUI は
                // ICoreWebView2_16 で追加された。QueryInterface で降りる必要がある。
                // クロージャは Send + 'static のため、macOS 側と同じく内部の失敗を
                ...
                unsafe {
                    ...
                    // BROWSER は WebView2 自身の印刷プレビューを出す。Edge の Ctrl+P と同じで、
...
fn build_menu<R: tauri::Runtime>(
    ...
    let reload_item = MenuItemBuilder::with_id("reload", tr(locale, Key::Reload))
        ...
        .build(app)?;
    // quit_with_text() は AppKit の terminate: にマップされ、windowShouldClose: を
    ...
    let quit_item = MenuItemBuilder::with_id("quit", tr(locale, Key::Quit))
        ...
        .build(app)?;
    // macOS は先頭の submenu をアプリケーションメニューとして扱い、名前も
...
// Windows にアプリケーションメニューという枠は無く、綴りもニーモニック（&）も macOS と
// 違うので構成ごと分ける。Window サブメニューは作らない：Minimize の既定加速キーが
// 無条件 Ctrl+M、Maximize は元に戻せない一方向で、どちらも Windows の慣習に無い。
#[cfg(target_os = "windows")]
fn build_menu<R: tauri::Runtime>(
    app: &tauri::AppHandle<R>,
    locale: menu_i18n::Locale,
    ...
    let reload_item = MenuItemBuilder::with_id("reload", tr(locale, Key::WinReload))
        .accelerator("F5")
        .build(app)?;
    // 定義済みの Fullscreen は Windows で何もせず、しかも定義済み項目は MenuEvent を
...
fn set_ui_locale(app: tauri::AppHandle, locale: String) {
    ...
    #[cfg(not(desktop))]
    {
        // iOS/iPadOS には tauri::menu 自体が無い。web 側は同じ呼び出しをするので受けるだけ。
        let _ = (&app, &locale);
    }
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // メニューは desktop 専用。iOS/iPadOS には tauri::menu 自体が無い。
            #[cfg(desktop)]
            {
                ...
                app.on_menu_event(move |app, event| {
                    ...
                    match event.id().as_ref() {
                        "print" => {
                            // AppKit のモーダルを回すので、メニューのコールバックから抜けてから実行する。
...
mod tests {
    ...
    // 属性が run() から離れると iOS だけ「required runtime symbols が無い」で落ち、desktop の
    // check も Windows の CI も通ってしまう（#88）。Apple SDK が要らない形で並びだけ見る。
    #[test]
