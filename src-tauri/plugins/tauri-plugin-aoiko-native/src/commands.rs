...
// iOS の解決結果は file:// URL、デスクトップは素のパス。存在確認も入出力もパスで行う。
...
pub(crate) fn confirm_discard<R: Runtime>(
    ...
    // iOS/iPadOS はウィンドウを閉じる操作もメニューの再読み込みも無く、これを呼ぶ
...
//
// iOS も同じ経路で足りる。resolved_dir が file:// URL を実パスへ解いており、Swift 側は
// resolveBookmark で取った security-scoped のスコープを選び直しまで手放さないので、
// Rust の std::fs はそのスコープの下で走る。バイト列を Swift へ渡す必要は無い。
...
// 行き先の rid だけをヘッダーで渡す。
//
// 生バイトが届かない経路もある。Tauri の custom protocol IPC が一度でも失敗すると
...
// iOS/iPadOS には保存先を選ばせる仕組みが無く、plugin-dialog の save() は空ファイルを
