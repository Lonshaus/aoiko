...
- Data is stored in your browser's IndexedDB and is **completely lost when you clear browser cache or site data**. The wrapper version (app) stores data the same way, so deleting the app's data loses it just the same.
...
- **Tracking prevention is on by default in the wrapper version (app) too.** Being an app does not mean the data is safe from it.
