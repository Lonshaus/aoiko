...
- The return side carries the filer info (tax office, user ID, name, address) and the business **revenue and income** (Blue Return only: also the **blue-return special deduction**). **Income deductions and tax computation are out of aoiko's scope** — complete them yourself in e-Tax. The financial-statement side carries the P/L, balance sheet, and monthly sales (purchases) — the White Return breakdown statement does not include a balance sheet.
