...
window.__aoikoNative = {
  // デスクトップは保存ダイアログ、iOS はアプリの Documents 直下。どちらを通るかは
  // プラグイン側だけが決める（iOS には保存先を選ばせる仕組みが無い）。
...
window.fetch = function (input, init) {
  ...
  // Windows の IPC は http://ipc.localhost を通るため isExternal では外部扱いになる。素の
  // fetch へ戻さないと invoke がこの上書きを再入して止まらない（macOS の ipc://localhost は
  // protocol の時点で外れる）。
...
// iOS/iPadOS は SFSafariViewController でアプリの上に重ねる。Safari へ飛ばすと
...
// 3. ネイティブのウィンドウ終了要求を web 版の未保存ガードへ繋ぐ。Tauri の終了では beforeunload が
...
// CLOSE_SCRIPT が未保存ガードを経ずに直接終了するときの入口。以前は withGlobalTauri で
...
// 4. 印刷をネイティブへ回す。WKWebView の window.print() は例外を投げるため（tauri#3066）、
//    請求書の印刷ボタンを押してもダイアログが開かず、未捕捉例外のバナーが出るだけだった。
//    印刷スタイルは公開 repo に揃っているので、出力は WKWebView 自身に描かせる。
window.print = function () {
  void (async () => {
    // デスクトップの print_page は WKWebView / WebView2 をそれぞれ直接叩く実装で、
    // iOS には無い。iOS は plugin 側の UIPrintInteractionController へ回す。
    await invoke((await isIos()) ? 'plugin:aoiko-native|print_page' : 'print_page');
  })();
};
// 5. 永続化ストレージの判定をこの環境の実情に合わせる。WebKit の persist() は免除リスト
